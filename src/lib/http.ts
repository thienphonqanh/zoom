...
const request = async <Response>(
  ...
  // Nếu không truyền baseUrl (hoặc baseUrl = undefined) thì lấy từ envConfig.NEXT_PUBLIC_API_END_POINT
  ...
  if (isClient) {
    ...
    if (normalizeUrl === 'api/auth/login') {
      const { accessToken, refreshToken } = (payload as ResponseDataType<LoginResponse>).data!
